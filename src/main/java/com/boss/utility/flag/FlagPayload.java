...
/**
 * Pure builder for the BossUtility flags webhook payload (kick / packet-kick / crash reports). No Minecraft
 * types here so it is unit-testable in isolation — {@link FlagReporter} extracts plain strings from the live
 * client and hands them in.
 *
 * <p>Reports the event type, reason, the reporting client's own Minecraft username, and the enabled
 * modules — <b>never a server name or IP</b> (neither is accepted as a parameter). Injection safety: every
 * untrusted string is put into a Gson object graph and serialized by Gson — never concatenated into
 * hand-built JSON or a shell command — plus we strip control characters, neutralize
...
public final class FlagPayload {
    ...
    public static final String BOUNDARY = "----BossUtilityFlagBoundary7MA4YWxkTrZu0gW";
    ...
    public enum Type {
        ...
        CRASH("Crash", 0xC0392B);                                  // harsh red
     ...
     * may be empty. {@code isoTimestamp} should be an ISO-8601 string. {@code username} is the reporting
     * client's own Minecraft name; there is deliberately no server parameter — server name/IP is never
     * collected. The recent-log excerpt is NOT part of this embed — it ships as a separate {@code .txt}
     * attachment via {@link #multipartBody}.
     */
    public static String build(Type type, String reason, String username, List<String> modules,
        ...
        payload.addProperty("username", "BossUtility Flags");
